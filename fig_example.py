...
def show_example(name_frame="F(1a)", name_scene="E(1)", N=1000, N_supression=0):
    ...
    for i_f, f in enumerate(["scene", "frame", "exbl"]):
        ...
        if f == "scene":
            ...
            plt.rcParams.update({"font.size": 14})
            im = plt.imshow(
                res["pred_1"], cmap="Reds", interpolation="nearest", aspect="auto"
            )
...
def show_stats(
    name_frame="F(1a)", name_scene="E(1)", N=1000, N_supression=0, is_pr=True, show=True
):
    """
    ...
    if show:
        ...
        plt.savefig(
            "res\example_{}.pdf".format("pr" if is_pr else "roc"),
            bbox_inches="tight",
            pad_inches=0,
        )
    plt.close()
