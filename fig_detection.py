...
def show_coloured_result(
    name_frame, name_scene, code, N=1000, N_supression=0, show=True
):
    """
    ...
    for i_f, f in enumerate(["scene", "frame", "exbl"]):
        fname = "res/{}_{}_{}_{}_{}.npz".format(
            name_frame, name_scene, N, N_supression, f
        )
        res = np.load(fname)
    ...
    final_res[
        np.logical_and(
            np.logical_and(results["gt"] == 1, results["exbl"] == 0),
            results["scene"] == 1,
        )
    ] = 2  # FN1 (orange)
    final_res[
    ...
    ] = 3  # FN2 (blue, not possible)
    final_res[
        np.logical_and(results["gt"] == 0, results["exbl"] == 1)
    ] = 4  # FP (light grey)
    final_res[
        np.logical_and(
            np.logical_and(results["gt"] == 0, results["exbl"] == 0),
            results["scene"] == 1,
        )
    ] = 5  # FP by ideal but mot exbl (green)
...
def show_sensitivity_analysis(N_supression=0, is_pr=True, show=True):
    ...
    for i_im, im in enumerate(images_n):
        ...
        for N in val_N:
            ...
            fname = "res/{}_{}_{}_{}_{}.npz".format(
                name_frame, name_scene, N, N_supression, "exbl"
            )
            res = np.load(fname)
            pred = res["pred_2"]
            if is_pr:
    ...
    if show:
        ...
        plt.savefig(
            "res/san_{}_{}.pdf".format(N, "pr" if is_pr else "roc"),
            bbox_inches="tight",
            pad_inches=0,
        )
    plt.close()
...
def show_stats(
    ...
    if show:
        ...
        plt.savefig(
            "res/res_{}_{}_{}.pdf".format(code, N, "pr" if is_pr else "roc"),
            bbox_inches="tight",
            pad_inches=0,
        )
    plt.close()
...
def show_table_row(name_frame, name_scene, code, N=1000, N_supression=0):
    ...
    for i_f, f in enumerate(["scene", "frame", "exbl"]):
        ...
        sss += "{:0.2f}&".format(vauc)
        if f == "exbl":
            vauc, _, _ = comp_pr(res["pred_2"], res["anno"])
            sss += "{:0.2f}\\NN".format(vauc)
    print(sss)
...
def compare_in_columns(Ns=[750, 1000, "10p", "20p"]):
    ...
    for im in IMAGES:
        ...
        for N in Ns:
            fname = "res/{}_{}_{}_{}_{}.npz".format(
                im["name_frame"], im["name_scene"], N, 0, "exbl"
            )
            res = np.load(fname)
            vauc, _, _ = comp_pr(res["pred_2"], res["anno"])
            ss += "&{:0.2f}".format(vauc)
